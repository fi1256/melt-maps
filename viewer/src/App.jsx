...
import {
  ActivityLayer,
  LAYER_IDS as ACTIVITY_LAYER_IDS,
  ACTIVITIES,
} from "./ActivityLayer";
import { LAYER_IDS as LPR_LAYER_IDS } from "./LprLayer";
import "maplibre-gl/dist/maplibre-gl.css";
import SettingsIcon from "./assets/settings.png";
import { ActivityLegend, ACTIVITY_ITEMS } from "./ActivityLayer/ActivityLegend";
...
const threatItem = ACTIVITY_ITEMS.find(
  (item) => item.simplified === "Threat to Observers"
);
const localPoliceItem = {
  label: "Local Police Present",
  icon: "icons/police.png",
};
const sheriffItem = {
  label: "Sherriff Present",
...
export default function App() {
  ...
  useMapPngImage("police", "./icons/police.png");
  useMapPngImage("sheriff", "./icons/sheriff.png");

  const cities = uniq(
    activityData.data?.geojson.features
    ...
    <MapProvider>
      ...
      >
          ...
          {!!popupFeature && popupFeature.properties && (
            ...
            >
              <h3 style={{ margin: "0 0 4px", fontSize: "14px" }}>
                {popupFeature.properties.simplified_activity}
              </h3>
              <Stack gap={1} direction="row" alignItems="center">
                {popupFeature.properties.threat_to_observers && (
                  <Tooltip title={threatItem.label} arrow>
                    <img
                      src={threatItem.icon}
                      alt={threatItem.label}
                      className="legend-image"
                    />
                  </Tooltip>
                )}

                {popupFeature.properties.local_police_present && (
                  <Tooltip title={localPoliceItem.label} arrow>
                    <img
                      src={localPoliceItem.icon}
                      alt={localPoliceItem.label}
                      className="legend-image"
                    />
                  </Tooltip>
                )}

                {popupFeature.properties.sherriff_present && (
                  <Tooltip title={sheriffItem.label} arrow>
                    <img
                      src={sheriffItem.icon}
                      alt={sheriffItem.label}
                      className="legend-image"
                    />
                  </Tooltip>
                )}
              </Stack>
              <b>Date:</b> {popupFeature.properties.activity_date}
              <br />
              <b>Time Start:</b> {popupFeature.properties.start_hour_min}
              <br />
              <b>Address:</b> {popupFeature.properties.address}
              <br />
              <b>Location Name:</b> {popupFeature.properties.location_type}
              <br />
              <b>License Plates:</b>{" "}
              {popupFeature.properties.ice_license_plates}
              <br />
              <b>People Taken:</b> {popupFeature.properties.people_taken}
              <br />
              <b>Agent Quantity:</b> {popupFeature.properties.agent_qty}
              <br />
              <b>Vehicle Quantity:</b> {popupFeature.properties.vehicle_qty}
              <br />
              <b>Additional Description:</b>{" "}
              {popupFeature.properties.additional_description}
            </Popup>
          )}
        </Map>
        ...
        >
          ...
          >
            updated{" "}
            {activityData.data?.updated
              ? new Date(activityData.data.updated).toLocaleString()
              : ""}
          </span>
        </div>
          ...
          <div id="map-legend">
            ...
            <div className="filter-section">
              <label className="filter-label">
                Hours {Math.round(hoursRange[0])} - {Math.round(hoursRange[1])}
              </label>
            ...
            <div className="filter-section">
              <label className="filter-label">
                ...
                <select
                  id="displayType"
                  onChange={(e) => setActivityDisplayType(e.target.value)}
            ...
            <div className="filter-section" style={{ maxWidth: "100%" }}>
              ...
              <CategoricalFilter
                ...
                setSelectedOptions={setSelectedCounties}
              />
            </div>
          </div>
        )}
